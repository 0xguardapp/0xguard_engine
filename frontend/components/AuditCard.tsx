...
export default function AuditCard({ audit }: AuditCardProps) {
  ...
  const auditLink = audit.targetAddress || audit.target || audit.id;
  
  return (
    <Link href={`/audit/${encodeURIComponent(auditLink)}`}>
      <div className="bg-[#09090b] border border-[#27272a] rounded-lg p-3 hover:border-gray-700 transition-all duration-200 cursor-pointer group">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-semibold text-sm tracking-tight group-hover:text-white transition-colors">
                {audit.name || truncateAddress(audit.targetAddress)}
              </h3>
              <p className="text-xs text-gray-500 mono">
                {audit.target || audit.targetAddress ? 
                  (audit.target || audit.targetAddress).slice(0, 8) + '...' + (audit.target || audit.targetAddress).slice(-6) :
                  'No target'
                }
              </p>
            </div>
        ...
        <div className="space-y-1.5 text-sm">
          ...
          <div className="flex items-center gap-3 pt-1.5 border-t border-[#27272a]">
